 ...
 * 
 * 运行方式：node migrate_sample_productId_fixed.js
 */
...
async function migrate() {
  console.log('=== 开始迁移样品 productId (修正版) ===');
  ...
  if (objectIdSamples.length === 0) {
    ...
    for (const sample of objectIdSamples) {
      ...
      if (tiktokProductId) {
        try {
          ...
          if (err.code === 11000) {
            stage1Duplicates++;
            console.log(`  [SKIP] 重复键: sample._id=${sample._id}, productId=${tiktokProductId}`);
          } else {
            throw err;
          }
        }
    ...
    console.log(`  阶段1跳过重复: ${stage1Duplicates} 条`);
  }
